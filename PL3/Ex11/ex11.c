...
int id=spawnChilds(10);
    ...
    for(int i=1;i<11;i++){
        ...
        shared_data->max[i-1]=max;
         ...
         close(down[LEITURA]);                                                                   //fechar a leitura

                write(down[ESCRITA], &shared_data->max[i],sizeof(shared_data->max[i-1]));//escrever no pipe           
    ...
    close(down[ESCRITA]);                                                                    //fechar a escrita

    while( (read(down[LEITURA], &valor, sizeof(valor)))>0){  //enquanto que existirem dados no pipe

		printf("Maior Numero da %dº Parte do array:%d\n",p,shared_data->max[p-1]);
        p++;
	}
